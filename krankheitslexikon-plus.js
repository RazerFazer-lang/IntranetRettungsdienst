(()=> {
  ...
  let DATA=[...BASE,...EXTRA.filter(x=>x&&typeof x==='object'&&x.name)];
  const PAGE_SIZE=96;
  let page=1;
  let currentQuery='';
  ...
  const ensureDataset=()=>{
    ...
    datasetPromise=new Promise((resolve,reject)=>{
      if(Array.isArray(window.RD_ICD10GM_ALL)){
        DATA=[...BASE,...EXTRA,...window.RD_ICD10GM_ALL];datasetReady=true;return resolve();
      }
      const s=document.createElement('script');s.src='./krankheitslexikon-all-2026.js';s.async=false;s.onload=()=>{DATA=[...BASE,...EXTRA,...(Array.isArray(window.RD_ICD10GM_ALL)?window.RD_ICD10GM_ALL:[])];datasetReady=true;resolve();};s.onerror=()=>reject(new Error('krankheitslexikon-all-2026.js konnte nicht geladen werden'));document.body.appendChild(s);
    });
  ...
  function draw(){
    ...
    document.getElementById('dxSearch').addEventListener('input',e=>{currentQuery=e.target.value;page=1;draw()});
    document.getElementById('dxCategory').addEventListener('change',e=>{currentCategory=e.target.value;page=1;draw()});
    document.getElementById('dxReset').addEventListener('click',()=>{currentQuery='';currentCategory='Alle';page=1;draw()});
    document.getElementById('dxResults').addEventListener('click',e=>{const b=e.target.closest('[data-dx-open]');if(b)open(b.dataset.dxOpen);});
    document.getElementById('dxResults').addEventListener('keydown',e=>{if(e.key==='Enter'){const card=e.target.closest('.disease-plus-card');if(card)open(card.dataset.id);}});
    document.querySelectorAll('.disease-page').forEach(b=>b.addEventListener('click',()=>{page=Number(b.dataset.page)||1;draw()}));
    document.getElementById('dxClose').addEventListener('click',()=>document.getElementById('dxModal').classList.add('hidden'));
    document.getElementById('dxModal').addEventListener('click',e=>{if(e.target.id==='dxModal')e.target.classList.add('hidden')});
  }
