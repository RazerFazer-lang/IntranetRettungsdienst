(()=> {
  ...
  const merge=official=>{
    const officialList=Array.isArray(official)?official.filter(x=>x&&x.code&&x.name):[];
    if(officialList.length===REQUIRED_TERMINAL_COUNT)return officialList;
    if(officialList.length>0)throw new Error(`Unvollständiger ICD-10-GM-Katalog: ${officialList.length} statt ${REQUIRED_TERMINAL_COUNT} Einträge.`);
    throw new Error(`Vollständiger ICD-10-GM-Katalog nicht verfügbar: erwartet werden ${REQUIRED_TERMINAL_COUNT} Einträge.`);
  };
  const ensureDataset=()=>{
    ...
    datasetPromise=new Promise((resolve,reject)=>{
      const finish=()=>{
        try{
          ...
          const meta=window.RD_ICD10GM_META||{};
          if(Number(meta.terminalCount)!==REQUIRED_TERMINAL_COUNT)throw new Error(`ICD-10-GM-Metadaten ungültig: terminalCount=${meta.terminalCount||'unbekannt'}, erwartet ${REQUIRED_TERMINAL_COUNT}.`);
          if(String(meta.version)!=='2026')throw new Error(`ICD-10-GM-Version ungültig: ${meta.version||'unbekannt'}, erwartet 2026.`);
          datasetReady=true;
          resolve();
  ...
  window.addEventListener('click',e=>{const b=e.target.closest?.('.nav-item[data-view="krankheiten-plus"]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();render().catch(err=>{console.error(err);app.innerHTML=`<div class="suite-note suite-danger"><strong>Krankheitslexikon konnte nicht vollständig geladen werden.</strong><br>${esc(err.message||err)}<br>Es werden absichtlich keine unvollständigen Krankheitsdaten angezeigt.</div>`});document.querySelectorAll('.nav-item').forEach(x=>x.classList.toggle('active',x===b));const crumb=document.getElementById('breadcrumbCurrent');if(crumb)crumb.textContent='Krankheitsbilder';},true);
  nav();
})();
