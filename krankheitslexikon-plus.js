(()=> {
  ...
  const merge=official=>{
    ...
    for(const x of Array.isArray(official)?official:[]){if(x?.code&&x?.name)byCode.set(String(x.code),x)}
    const fallback=[...BASE,...EXTRA];
    for(const x of fallback){
      if(!x?.name)continue;
      if(x.code&&byCode.has(String(x.code)))byCode.set(String(x.code),{...byCode.get(String(x.code)),...x});
      else if(x.code)byCode.set(String(x.code),x);
      else byCode.set(String(x.id||x.name),x);
    }
  ...
  const ensureDataset=()=>{
    ...
    datasetPromise=new Promise((resolve,reject)=>{
      const finish=()=>{
        DATA=merge(window.RD_ICD10GM_ALL);
        datasetReady=true;
        resolve();
      };
