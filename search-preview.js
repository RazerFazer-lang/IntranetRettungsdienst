(() => {
  ...
  function getResults(query='') {
    if (typeof RD_DATA === 'undefined' || !Array.isArray(RD_DATA.common)) return [];
    const q = query.trim().toLowerCase();
    if (!q) return RD_DATA.common.slice(0, 6);
    return RD_DATA.common
      .filter(x => [x.title, x.cat, x.desc].some(v => String(v).toLowerCase().includes(q)))
      .slice(0, 7);
  }
  ...
  function renderPreview(input, showEmpty=false) {
    ...
    box.innerHTML = `${!q ? '<div class="live-preview-title">Schnelltreffer</div>' : ''}` +
      results.map((x, i) => `
        <button type="button" class="live-result" data-live-guide="${esc(x.id)}">
          <span class="live-icon">${icons[i % icons.length]}</span>
          <span class="live-copy">
            <strong>${esc(x.title)}</strong>
            <small>${esc(x.cat)} · ${esc(x.desc)}</small>
          </span>
          <span class="badge ${x.priority === 'Kritisch' ? 'red' : ''}">${esc(x.priority)}</span>
        </button>`).join('');
    box.classList.remove('hidden');
  }

  function openResult(id, input) {
    if (typeof openGuide === 'function') openGuide(id);
    else document.querySelector(`[data-guide="${CSS.escape(id)}"]`)?.click();
    ensurePreview(input).classList.add('hidden');
  }
  ...
  function wireInput(input) {
    ...
    input.addEventListener('keydown', e => {
      ...
      if (e.key === 'Enter') ensurePreview(input).classList.add('hidden');
    });
  }
  ...
  document.addEventListener('click', e => {
    ...
    if (hit) {
      const input = hit.closest('.search')?.querySelector('input');
      openResult(hit.dataset.liveGuide, input);
      return;
    }
